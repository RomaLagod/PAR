  ...
  Begin
   //Обчислення відгалужень
     ...
     // showmessage(connectV);
       For fc:=1 to InputData.RowCount-1 do
        Begin
...
Begin
    ...
    Begin
       if STGrid.Cells[3,i]<>'' then
         ...
         Begin
            MessageDLG('Комірка з даними виду місцевого опору порожня!',mtError,[mbOk],0);
         end;{else}
       end;{for}
 ...
 // if STP<0.1 then STP:=0.1;
  ...
  for i:=1 to super_i do
   ...
   If (inter_a = (-1)) or (inter_b = (-1)) or (inter_c = (-1)) or (inter_d = (-1)) or (STP>3600000) or (STT>400) or (STT<120) or (STP<=100000) then
    case messageDLG('Перевірте параметри пари.',mtError,[mbOK],0) of
...
{    Діаметри         ------------------------Надземне прокладання--------------    -----------------------В непрохідних каналах------------    --------------В прохідних каналах----------------------}
   ...
   DqT[0,14]:=0.307 ;   DqT[1,14]:=1.24 ;   DqT[2,14]:=1.22 ;   DqT[3,14]:=1.17 ;   DqT[4,14]:=1.20 ;   DqT[5,14]:=1.17 ;   DqT[6,14]:=1.09 ;   DqT[7,14]:=1.32 ;   DqT[8,14]:=1.38 ;   DqT[9,14]:=1.19 ;
   DqT[0,15]:=0.309 ;   DqT[1,15]:=1.24 ;   DqT[2,15]:=1.22 ;   DqT[3,15]:=1.17 ;   DqT[4,15]:=1.20 ;   DqT[5,15]:=1.17 ;   DqT[6,15]:=1.09 ;   DqT[7,15]:=1.32 ;   DqT[8,15]:=1.38 ;   DqT[9,15]:=1.19 ;
