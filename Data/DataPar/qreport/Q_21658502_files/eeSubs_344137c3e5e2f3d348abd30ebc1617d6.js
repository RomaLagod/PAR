...
function isElementVisible(elementID)
{
   ...
   if (element != null && element.style.visibility != 'hidden' && element.style.display != 'none')
   {
      return true;
   } 
   else
...
function scrollToElement(element)
{
   ...
   var position = getElementPosition(element);
      //In IE8 scrolls to the x coordinate and shifts the page without 
      //allowing the user to scroll back to the original spot
      //changed it so that the function leave the x coordinate alone.
...
function hideInfoBoxOverlay(id, hackFooter, infoBoxBinId)
{
   var infoBox = document.getElementById(id);
   
   if (infoBoxBinId)
   {
      var infoBoxBin = document.getElementById(infoBoxBinId);
      infoBox.style.display = 'none';
      infoBoxBin.appendChild(infoBox);
   }
   
   hidePopupOverlay(id, hackFooter);
}
...
// LOGIN OVERLAY SCRIPT FOR SURVEY OVERLAYS
function buildSurveyAnswer(questionType)
{
   var UNKNOWN = 0;
   var CHECK_BOX = 1;
   var RADIO_BUTTON = 2;
   ...
   var DATE = 7;
   var RATING = 8;
   var RADIO_CHECK_BOX = 9;
   var RADIO_WITH_TEXT = 10;
   var CHECK_BOX_WITH_TEXT = 11;
   ...
   // NOTE: CURRENTLY ONLY SUPPORTING THE QUESTION TYPES THAT I NEED
   if(questionType == TEXT_AREA)
   {
      var textArea = document.getElementById("option1");
      ...
      if(textArea != null)
      {
         ...
         {
            answer = "C" + "," + sanitizeSurveyAnswer(textArea.value, [HYPHEN]);
         }
      }
   }
   ...
   {
      var first = true;
      var subQuestions = document.getElementsByName("subQuestions");
      
      for(i = 0; i < subQuestions.length; i++)
      {
         var subQuestion = subQuestions[i];
         var subQuestionID = subQuestion.id;
         var subQuestionIDCode = subQuestionID.replace("subQuestion", "");
         ...
         answer = answer + String.fromCharCode(subQuestionIDCode);
         
         options = document.getElementsByName(subQuestionID);
         
         var pos;
         var answered = false;
         
         for(pos = 0; pos < options.length && !answered; pos++)
         {
   ...
   else if(questionType == CHECK_BOX || questionType == RADIO_CHECK_BOX 
      || questionType == CHECK_BOX_WITH_TEXT)
   {
      var noneOption = document.getElementById("noneOption");
      
      if(noneOption != null && noneOption.checked)
      {
         answer = answer + noneOption.value;
      }
      else
      {
         var first = true;
         
         options = document.getElementsByName("checkOptions");
         ...
         for(i in options)
         {
            ...
            if(checkbox.checked)
            {
               ...
               if(questionType == CHECK_BOX_WITH_TEXT)
               {
                  var textFieldID = checkbox.id + "Text";
                  var textField = document.getElementById(textFieldID);
                  
                  if(textField != null)
                  {
                     answer = answer + "," + sanitizeSurveyAnswer(textField.value, [HYPHEN]);
                  }
               }
            }
         }
   ...
   else if(questionType == RADIO_BUTTON || questionType == RADIO_WITH_TEXT)
   {  
      options = document.getElementsByName("radioOptions");
      
      var answered = false;
      
      for(i = 0; i < options.length && !answered; i++)
      {
         ...
         if(option.checked)
         {
            ...
            if(questionType == RADIO_WITH_TEXT)
            {
               var textFieldID = option.id + "Text";
               var textField = document.getElementById(textFieldID);
...
function radioCheckbox(name)
{
   var toggledCheckboxes;
   
   if(name == "checkOptions")
   {
      toggledCheckboxes = new Array();
      
      toggledCheckboxes[0] = document.getElementById("noneOption");
   }
   else if(name == "noneOption")
   {
      toggledCheckboxes = document.getElementsByName("checkOptions");
   }
   
   var i;
   for(i in toggledCheckboxes)
   {
      var checkbox = toggledCheckboxes[i];
...
function textOptionFocus(optionID)
{
   var textID = optionID + "Text";
   
   var textInputField = document.getElementById(textID);
...
function clearTextField(fieldID, defaultText)
{  
   var textInputField = document.getElementById(fieldID);
   
   if(textInputField.value == defaultText)
   {
      textInputField.value = "";
   }
}

function toggleSurveyContent(browser, frame, surveyID, bodyFrameTotal)
{
   var callback = null;
   
   var url = "/shared/async/surveyFrame.jsp";
   var commonParameters = "surveyID=" + surveyID + "&sectionTotal=" + bodyFrameTotal;
   
   if(frame == "completed")
   {
      var parameters = commonParameters + "&completed=true";
      
      var omnitureAnswers = "";
      
      var i;
      for(i = 0; i < bodyFrameTotal; i++)
      {
         ...
         if(answer != null)
         {
            ...
            parameters = parameters + "&" + answerID + "=" + answer;
            
            var delimiter = answer.indexOf(",");
            
            if(delimiter != -1)
            {
               answer = answer.substring(0, delimiter);
            }
            
            if(i == 0)
            {
               omnitureAnswers = answer;
            }
            else
            {
               omnitureAnswers = omnitureAnswers + " " + answer;
            }
         }
      }
      ...
      callback = function() {
         ...
         if(browser == "IE" || browser == "IE7" || browser == "IE8")
         {
            isIE = true;
         }
         
         postAsyncOmnitureProp("eVar38", omnitureAnswers);
         hidePopupOverlay("psSurvey", isIE);
      };
      
      new AsyncPost(url, null, parameters, callback);
   }
   else
   {
      var contentID = "surveySectionContent";
      
      updateSurveyTitle(frame, bodyFrameTotal);
      
      //updateSurveyCloseButton(browser, surveyID, bodyFrameTotal);
      
      url = url + "?" + commonParameters + "&section=" + frame;
      
      if(frame > bodyFrameTotal)
      {
         callback = function() {
            postAsyncOmnitureEvent("event77");
         };
      } 
      else
      {
         callback = function() {
            postAsyncOmnitureEvent(null);
         };
      }
      
      AsyncInclude(url, contentID, callback, true, browser);
   }
}

function undoSurveyAnswer(eVarCurrentValue)
{
   var lastAnswerStartIndex = eVarCurrentValue.lastIndexOf("Q");
   
   var eVarNewValue = eVarCurrentValue.substring(0, lastAnswerStartIndex);
   
   return eVarNewValue;
}

function updateSurveyCloseButton(browser, surveyID, bodyFrameTotal)
{
   var closeButton = document.getElementById("infoBoxCloseButton");
   
   // The close button should never be null.
   if(closeButton != null)
   {
      closeButton.onclick = function() {
         toggleSurveyContent(browser, "completed", surveyID, bodyFrameTotal);
      };
   }
}

function updateSurveyTitle(frame, maxBodyFrame)
{
   ...
   else if(frame > maxBodyFrame)
   {
      titleElement.innerHTML = "Survey Complete";
   }
   else
   {
      titleElement.innerHTML = "Question " + frame + " of " + maxBodyFrame;
   }
}

function sanitizeSurveyAnswer(answer, toReplace)
{
   var DBL_QUOTE = 34;
   var HYPHEN = 45;
   var SINGLE_QUOTE = 96;
   
   var sanitizedAnswer = answer;
   
   // Loops over the characters that need to be replaced.
   var tR;
   for(tR = 0; tR < toReplace.length; tR++)
   {
      // Break the answer around the characters to be replaced.
      var answerPieces = 
         sanitizedAnswer.split(String.fromCharCode(toReplace[tR]));
      ...
      // Rebuild the answer with the replaced characters.
      var aP;
      for(aP = 0; aP < answerPieces.length; aP++)
      {
         if(aP == 0)
         {
            sanitizedAnswer = answerPieces[aP];
         }
         else
         {
            sanitizedAnswer = sanitizedAnswer + replacement + answerPieces[aP];
         }
      }
   }
...
function storeSurveyAnswer(questionID, questionType)
{  
   var DBL_QUOTE = 34;
   var SINGLE_QUOTE = 96;
   
   var answer = "Q" + questionID + ":" + buildSurveyAnswer(questionType);
      
   var storedAnswer = document.createElement("input");
   storedAnswer.id = "answer" + questionID;
   storedAnswer.name = "storedAnswer";
   storedAnswer.type = "hidden";
   storedAnswer.value = sanitizeSurveyAnswer(answer, [DBL_QUOTE, SINGLE_QUOTE]);
      
   var storedAnswerSection = document.getElementById("storedAnswerSet");
      
   storedAnswerSection.appendChild(storedAnswer);
}

function postAsyncOmnitureEvent(value)
{
   // event4 is for all page views and thus occurs on every page.
   var baseEvent = "event4";
   
   var s2 = getOmnitureFunction();
   
   if(value != null)
   {
      s2.events = baseEvent + "," + value;
   }
   else
   {
      s2.events = baseEvent;
...
/// EE Prompt //
var _eePromptValue;
var _eePromptCallbackFunction;

function harvestAndClosePrompt(inputId, promptId, promptBinId, hackFooter)
{
   _eePromptValue = document.getElementById(inputId).value;
   hideInfoBoxOverlay(promptId, hackFooter, promptBinId);
   eval(_eePromptCallbackFunction);
}

function showEEPrompt(promptTitle, labelValue, initialValue, callbackFunction, hackFooter, hackOverlay, browser)
{
   document.getElementById('eepTitle').innerHTML = promptTitle;
   document.getElementById('eepLabel').innerHTML = labelValue;
   _eePromptValue = "";
   _eePromptCallbackFunction = callbackFunction;
   showInfoBoxOverlay('eePrompt', hackFooter, hackOverlay, browser);
   document.getElementById('eepInput').focus();
   document.getElementById('eepInput').value = initialValue;   
   document.getElementById('eepInput').select();
}

function getEEPromptValue()
{
   return _eePromptValue;
}

/// end EE Prompt code//
function promoteContent(eeRefID, refID, taID)
{
   AsyncInclude('/shared/async/landingPageCandidateAdder.jsp?param&eerfid='+eeRefID+'&refID='+refID+'&taID='+taID+'&action=1', null, null);
   document.getElementById('promoteButton').className='initiallyHidden'; 
   document.getElementById('promotePrompt').className='tinyPromoteButtonBlank';
}
