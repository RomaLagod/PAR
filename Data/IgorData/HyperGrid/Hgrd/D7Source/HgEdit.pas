...
//		here it makes the combobox only read only - tsoyran dokimastika
		SendMessage( Handle , EM_SETREADONLY , 1 , 0 )
	else
		...
		if Accept then
			if EditCanModify and ( not Column.ReadOnly ) then
				with THyperGrid(Grid) do
					begin
						Cells[ Col , Row ] := Value;
						DoComboSelection( Col , Row,FPicklist.ItemIndex );
					end;
	end;
end;
...
begin
	...
	begin
		Column := VisibleColumns[ Col ];
		isDropDown := ( HgGlobal.hgctlComboBox in Column.ControlType ) and Column.DropDownList;
	end;
	...
	case Key of
		...
		VK_RETURN, VK_ESCAPE:begin
			...
			if isDropDown and not FListVisible then DropDown;
	end;
end;
...
procedure ThgInPlaceEdit.KeyPress(var Key: Char);
var tftt : ThgFormatTextType;
		tft  : ThgFormatType;   // = ( hgfmtNone
		tct  : ThgControlTypes;
begin
	tftt:= THyperGrid( Grid ).Columns[THyperGrid( Grid ).Col].FormatText;
	tft := THyperGrid( Grid ).Columns[THyperGrid( Grid ).Col].FormatType;
	tct	:= THyperGrid( Grid ).Columns[THyperGrid( Grid ).Col].ControlType;
	if (tct=[]) then
	begin
		case tft of
			hgfmtNone :
					case tftt of
						hgfmtAsUpcase : key := char(CharUpper(PChar(key)));
						hgfmtAsLowered: key := char(CharLower(PChar(key)));
					end;
			hgfmtNumeric:
					if not (key in [#8,#13,#45,'0'..'9',DecimalSeparator]) then
						key:=#0
		end;
	end;
	inherited KeyPress(Key);
...
// set the kind of cursor depending on the kind of cell and
// in the area of cursor
	...
	begin
		...
		with THyperGrid( Grid ) do
			begin
				...
				begin
					Column := VisibleColumns[ Col ];
					index  := column.Items.IndexOf(Text);
					DoComboSelection( Col , Row, Index );
				end;
			end;
	end
end;
