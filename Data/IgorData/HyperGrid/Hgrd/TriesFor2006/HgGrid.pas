...
{
	HISTORY of changes - additions from
	tsoyran@otenet.gr
	users.otenet.gr/~tsoyran
	...
	==================================
		...
		Additions
			...
			portions of the code was in the SortGrid code
					Class: TSortGrid
					Copyright 1996,1997,1999
					Author: Bill Menees, bmenees@usit.net, www.public.usit.net/bmenees


			by with mouse and contrl - for selecting in row basis
...
type
	ThgOption = ( hgoptDrawFocusRectangle,
								hgoptHighlightSelection,
								hgoptSpecialSelection, {ver 2.22 RVS03}
								hgoptSkipReadOnly, 		 {ver 2.23 RVS04}
								hgoptCycleTravel			 {ver 2.23 RVS04});
  ...
  ThgOnExitCell = procedure( Sender : TObject; ACol , ARow : Longint;
	...
	ThgOnComboSelection = procedure( Sender : TObject; ACol , ARow, Index : Longint ) of object;

	ThgOnCellEdited = procedure( Sender : TObject; ACol , ARow : Longint ) of object;

  ThgOnBeforeFixedCellClick = procedure ( Sender : TObject; ACol , ARow , HeadingIndex : Longint; var CanClick : Boolean ) of object;
	ThgOnCheckedChange = procedure( Sender : TObject; ACol , ARow : Longint ) of object;
  ...
  private
    ...
    FAutoAcceptSuggestion   : Boolean;
		...
		fSelectedRows:           TList;    //RVS03
		fAnchor:                 LongInt;  //RVS03
    fLastMoveOn:             LongInt;  //RVS03
    fLastTopRow:             LongInt;  //RVS03
    fMouseIsDown:            Boolean;  //RVS03
    ...
    FOnBeforeFixedCellClick : ThgOnBeforeFixedCellClick;
		FOnCheckedChange 				: ThgOnCheckedChange;	//tsoyran
	private
    ...
    procedure SetOptions( Value : ThgOptions );
		...
		function GetLastVisibleColumn: ThgColumn; // tsoyran  ver 2.23  RVS04
		function GetFirstVisibleColumn: ThgColumn; // tsoyran  ver 2.23  RVS04

	private // start RVS03  2.12
		function  GetSelected( Row: LongInt ): Boolean; //RVS03
		procedure SetSelected( Row: LongInt; Select: Boolean ); //RVS03
		function  GetSelectedCount: LongInt; //RVS03
		procedure ToggleRow( aRow: LongInt ); //RVS03
		Procedure DelectedSelecteds; // //RVS03
		procedure SelectRow( aRow: LongInt; Select: Boolean ); //RVS03
		procedure SelectRows( aRow, bRow : LongInt; Select: Boolean ); //RVS03
		function  GetSelItems( Index: LongInt ): LongInt; //RVS03
	protected
		procedure TopLeftChanged; override; //RVS03
	public
		property  SelectedItems[ Index: LongInt ]: LongInt read GetSelItems; //RVS03
		property  Selected[ Row: LongInt ]: Boolean read GetSelected write SetSelected; //RVS03
		property  SelectedCount: LongInt read GetSelectedCount; //RVS03
		procedure ClearSelection; //RVS03

		procedure TransformRow(FromRow,ToRow:LongInt); // RVS04
	private
    ...
    procedure SetColCount( const C : Longint );
		procedure DoComboSelection( ACol , ARow,Index : Longint );
	private
...
{tsoyran}
	private
		procedure SetCheckedAndDraw(Acol,Arow:Longint;Checked:boolean);
	protected
		function IsCheckBoxCell(Coord : TGridCoord): boolean;
		function CanEditShow: Boolean; override;
		procedure DrawCheckBox(ARect : TRect;ACol:LongInt;BooleanType:ThgBooleanType; focus:boolean; Checked : Boolean );
	public
		function  IsChecked(Acol,Arow:Longint):boolean;
		procedure SetChecked(Acol,Arow:Longint;Checked:boolean);
...
{   Only needed for hint stuff
	...
	public
		...
		procedure DeleteRow( ARow : Longint ); reintroduce;//antonis
		...
		procedure SaveColsToFile( StartCol , EndCol, StartRow : Longint; const FileName : string );
		procedure LoadColsFromFile( StartCol , EndCol, StartRow : Longint; const FileName : string );
		procedure SaveColsToStream( StartCol ,EndCol, StartRow : Longint; Stream : TStream );
		procedure LoadColsFromStream( StartCol, EndCol, StartRow : Longint; Stream : TStream );
    ...
    property OnBeforeFixedCellClick : ThgOnBeforeFixedCellClick read FOnBeforeFixedCellClick write FOnBeforeFixedCellClick;
    //tsoyran
		property OnCheckedChange 				: ThgOnCheckedChange read FOnCheckedChange write FOnCheckedChange;
	end;
...
function THyperGrid.IsCheckBoxCell(Coord : TGridCoord): boolean;
begin
	result := false;
	if (Coord.X>=fixedCols) and (Coord.Y>=fixedRows) and (Coord.X< ColCount) and (Coord.Y<RowCount) then
		result := hgctlCheckBox in columns[Coord.X].ControlType
end;

function THyperGrid.IsChecked(Acol,Arow:Longint):boolean;
var coord: TGridCoord;
begin
	result := false;
	Coord.X := Acol;
	Coord.Y	:= Arow;
	if IsCheckBoxCell(Coord) then
		result := Assigned(Objects[ACol,ARow])
end;

procedure THyperGrid.DrawCheckBox(ARect : TRect;ACol:LongInt;	BooleanType:ThgBooleanType;
	focus:boolean; Checked : Boolean );
	Const
...
procedure THyperGrid.SetChecked(Acol,Arow:Longint;Checked:boolean);
begin
	if (ACol>=fixedCols) and (ARow>=fixedRows) and (ACol< ColCount) and (Arow<RowCount) then
		if Checked then
			Objects[ACol,ARow] := TObject(hgDefaultCheckInteger)
		else
			Objects[ACol,ARow] := Nil;
end;

procedure THyperGrid.SetCheckedAndDraw(Acol,Arow:Longint;Checked:boolean);
	var
		R	 : TRect;
		Column : ThgColumn;
begin
	SetChecked(Acol,Arow,Checked);
	if (ACol>=fixedCols) and (ARow>=fixedRows) and (ACol< ColCount) and (Arow<RowCount) then
	begin
		R := CellRect( ACol , ARow);
		Column := VisibleColumns[ ACol ];
		DrawCheckBox(r,ACol, Column.BooleanStyle,true,Checked);
		if assigned(fOnCheckedChange) then
			fOnCheckedChange( Self,ACol,ARow)
	end;
end;

function THyperGrid.CanEditShow: Boolean;
var	Column : ThgColumn;
begin
...
procedure THyperGrid.KeyDown(var Key: Word; Shift: TShiftState);
	var
		checked : boolean;
		IsReadOnly : boolean;
		Column : ThgColumn;
...
begin
	...
	oldCOl := Col;
	// RVS04 ver 2.23 --->>
	if (hgoptCycleTravel in FHyperOptions) then
	begin
		Column := VisibleColumns[ Col ];
		if (Column = GetLastVisibleColumn) and (key = VK_RIGHT) then
			key :=  VK_TAB;
		if (Column = GetFirstVisibleColumn) and (key = VK_LEFT) then
		begin
			key :=  VK_TAB;
	...
	begin
		...
		if Assigned(OnGetReadOnly ) then
			Self.OnGetReadonly( Self, col ,Row , IsReadOnly );
		...
		begin
			...
			begin
				...
				begin
					checked := not Assigned(Objects[Col,Row]);
					SetCheckedAndDraw(col,row,Checked);
				end;
			end
		end
	end;
	inherited KeyDown(Key, Shift);
		// RVS04 ver 2.23 --->>
	if (hgoptSkipReadOnly in FHyperOptions) then
	repeat
		Column 		 := VisibleColumns[ Col ];
		IsReadOnly := Column.ReadOnly;
		if Assigned(OnGetReadOnly ) then
...
begin
  ...
  FLastFocus.X := FixedCols;
	...
	fSelectedRows            := TList.Create; //RVS03
	fSelectedRows.Add( Pointer( FixedRows ) ); //RVS03
	fAnchor                  := FixedRows; //RVS03
	fMouseIsDown             := False; //RVS03
	fLastTopRow              := TopRow; //RVS03
...
//------------------------------------------------------------------------------
// returns the last,First visible ThgColumn object
// tsoyran
function THyperGrid.GetLastVisibleColumn: ThgColumn;
var
	ColIndex : Integer;
begin
	ColIndex := FColumns.Count - 1;
	while ( ColIndex >= 0 ) do
	begin
...
function THyperGrid.GetFirstVisibleColumn: ThgColumn;
var
	ColIndex : Integer;
begin
	ColIndex := FixedCols;// + 1;
	while ( ColIndex < FColumns.Count ) do
	begin
		if FColumns[ ColIndex ].Visible then
				break;
		inc( ColIndex );
	end;
	if ColIndex >= FColumns.Count then
		raise EhgColumnIndexOutOfRange.Create( 'THyperGrid: Column index out of range' )
	else
		Result := FColumns[ ColIndex ];
end;
...
begin
	...
	with DrawInfo do
		begin
			...
			// tsoyran
			if ( Column is THGColumn ) then
			begin
				with (Column as THGColumn) do
				begin
					if (hgctlCheckBox in ControlType) then
						if (BooleanStyle in [hgcbtCheckBox, hgcbtRadio]) then
							if IsChecked(Acol,Arow) then
								Text := 'Yes'
							else
								Text := 'No'
						else
	...
	// >> RVS03
	if ([goRangeSelect,goRowSelect] <= Options) and
		(hgoptSpecialSelection in HyperOptions) then // ver 2.22
	begin
		if not (gdFixed in AState) and (aRow <= RowCount - 1 ) then
		begin
			Exclude( CellState, hgcsSelected );
			if (fSelectedRows.IndexOf( Pointer( aRow ) ) > -1) and
				 (hgoptHighlightSelection in HyperOptions ) then
			begin
				Include(CellState, hgcsSelected );
				AColor := FHighlight;
...
{start Tsoyran}
	if not ( gdFixed in AState ) and (hgctlCheckBox in Column.ControlType) then
	begin
		checked := Assigned(Objects[ACol,Arow]);
		DrawCheckBox(ARect,ACol,Column.BooleanStyle,
			(hgcsSelected in CellState)
			{( gdFocused in AState ) or (gdSelected in AState)},checked);
	end;
{end tsoyran}
	...
	try
		...
		begin
			case Column.FormatType of
				hgfmtNone		 :
					if Column.FormatText<>hgfmtAsEntered then
					begin
						case Column.FormatText of
							hgfmtAsLowered: Result := AnsiLowerCase(result);
							hgfmtAsUpcase	: Result := AnsiUpperCase(result);
							hgfmtAsName		: Result := Char(charUpper(Pchar(result[1])))+Copy(AnsiLowerCase(result),2,length(result));
						end;
//						Cells[ ACol , ARow ]:= result;
					end;
				hgfmtNumeric :
					begin
					// tsoyran ----------
						if not texttofloat(pchar(Result),FloatValue,fvExtended) then
							FloatValue := 0;
					// ---------- tsoyran
				...
				hgfmtDateTime :
					begin
						...
						begin
							...
							if Column.DateFormat >= 0 then
							begin
								DateTime := StrToDate( Result );
								Format := hgDateFormats[ hgDateOrder , Column.DateFormat ];
							end
							else
								if Column.TimeFormat >= 0 then
								begin
...
begin
	Coord := HgMouseCoord( X , Y );
	// tsoyran
	if (ssDouble in Shift) and IsCheckBoxCell(Coord) then
	begin
		key := VK_SPACE;
		KeyDown(key,Shift);
	end;
...
// New for version 1.5
		...
		begin
			...
			if Assigned( FOnBeforeFixedCellClick ) then
				FOnBeforeFixedCellClick( Self , FTrackCol , FTrackRow , FTrackHeading , FTracking );
			if FTracking then
			begin
				FButtonDown := Button;
				MouseCapture := True;
	...
	if (hgoptSpecialSelection in HyperOptions) and // ver 2.22
		(Button = mbLeft) and (goRangeSelect in Options) then // RVS03
	begin
		aRow := Coord.Y;
...
//  HintMouseMove( X , Y );
  ...
  //RVS03
	if fMouseIsDown and (hgoptSpecialSelection in HyperOptions) then // ver 2.22
    begin
...
procedure THyperGrid.DoComboSelection( ACol , ARow, index : Longint );
begin
	if Assigned( FOnComboSelection ) then
		FOnComboSelection( Self , ACol , ARow,index );
end;
...
//------------------------------------------------------------------------------
// Version 2.11 Tsoyran

procedure THyperGrid.LoadColsFromStream( StartCol, EndCol, StartRow : Longint; Stream : TStream );
	...
	begin
		...
		while P^ <> #0 do
			begin
				...
				if hgctlCheckBox in Columns[ SCol ].ControlType   then
				begin
					if S = '0'
					then
						Objects[ GridColumnForColumnObject( Columns[ SCol ] ) , ToRow ] := Nil
					else
						Objects[ GridColumnForColumnObject( Columns[ SCol ] ) , ToRow ] := Pointer(1)
				end
				else
				...
				Inc( SCol );
				if (SCol = Columns.Count) or (SCol > EndCol) then
				begin
					break;
				end;
...
procedure THyperGrid.SaveColsToStream( StartCol, EndCol, StartRow : Longint; Stream : TStream );
var
	Lines : TStringList;
	SCol , SRow : Longint;
	Line , S : string;
begin
	...
	try
		for SRow := StartRow to Pred( RowCount ) do
			begin
				Line := '';
				for SCol := StartCol to EndCol do
					begin
						if hgctlCheckBox in Columns[ SCol ].ControlType   then
						begin
							if Objects[ GridColumnForColumnObject( Columns[ SCol ] ) , SRow ] = Nil then
								S:= '0'
							else
								S:= '1'
						end
						else
							if SRow = 0 then // ver 2.22
								S := Columns[ SCol ].Caption
							else
...
procedure THyperGrid.LoadColsFromFile( StartCol , EndCol, StartRow : Longint; const FileName : string );
var
	Stream : TFileStream;
begin
	Stream := TFileStream.Create( FileName , fmOpenRead );
	try
		LoadColsFromStream( StartCol , EndCol, StartRow , Stream );
	finally
		Stream.Free;
	end;
end;

procedure THyperGrid.SaveColsToFile( StartCol , EndCol, StartRow : Longint; const FileName : string );
var
	Stream : TFileStream;
begin
	Stream := TFileStream.Create( FileName , fmCreate );
	try
		SaveColsToStream( StartCol , EndCol, StartRow , Stream );
	finally
		Stream.Free;
	end;
end;
...
// RVS03
procedure THyperGrid.ClearSelection; //RVS03
begin
	fSelectedRows.Clear;
...
Procedure THyperGrid.DelectedSelecteds; // //RVS03
begin
	fSelectedRows.Clear;
	Refresh;
end;

function THyperGrid.GetSelected(Row: Integer): Boolean;
begin
	Result := fSelectedRows.IndexOf( Pointer( Row ) ) > -1 ;
end;

function THyperGrid.GetSelectedCount: LongInt;
begin
	Result := fSelectedRows.Count;
end;

function THyperGrid.GetSelItems( Index: LongInt ): LongInt; //RCV03
begin
	 Result := LongInt( fSelectedRows.Items[ Index - 1 ] );
end;

procedure THyperGrid.SelectRow(aRow: Integer; Select: Boolean);
var
	ListIndex: Integer;
begin
...
procedure THyperGrid.SelectRows(aRow, bRow: Integer; Select: Boolean);
var
...
procedure THyperGrid.SetSelected(Row: Integer; Select: Boolean);
begin
	SelectRow( Row, Select );
end;

procedure THyperGrid.ToggleRow(aRow: Integer);
var
	Index: Integer;
begin
	Index := fSelectedRows.IndexOf( Pointer( aRow ) );
...
procedure THyperGrid.TopLeftChanged;
begin
...
//------------------------------------------------------------------------------
// Version 2.21
procedure THyperGrid.TransformRow(FromRow, ToRow: Integer); //RVS04
begin
	if (FromRow>FixedRows)  and (FromRow < RowCount) and
	 (FromRow<>ToRow) and (ToRow>FixedRows) and (ToRow<RowCount) then
